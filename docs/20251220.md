...
**問題の根本原因:**
- CIでは `npx prisma generate` を実行してからlint
- ローカルのpre-commitでは `prisma generate` を実行していなかった
- スキーマ変更後も古いPrismaクライアントがキャッシュされてlintが通ってしまった

**対策:**
`.husky/pre-commit` を修正し、`schema.prisma` がステージングされている場合は自動で `prisma generate` を実行するようにした。

```bash
# schema.prismaが変更されていたらPrismaクライアントを再生成
...
これでスキーマ変更時はローカルでもCIと同じ条件でlintが実行される。
